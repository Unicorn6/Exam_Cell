...
foreach($loadedSheetNames as $sheetIndex => $loadedSheetName) {
	...
	$n=$id['subid'];
	echo $n."----";
    $u=$db->query("SELECT drawhall FROM SUBJECT WHERE subid='$n'");
	$out=$u->fetch_assoc();
	$dh=$out['drawhall'];
	$count=0;
	$flag=0;
...
if($handle=fopen("temp.csv","r"))
{
	while($data=fgetcsv($handle,1000,",")){
		$count++;
		if($count>2){
			$qw=$db->query("INSERT INTO `$n`(`SID`,`DEPT`,`DHALL`) VALUES ('$data[1]',$dept,$dh)");
			echo $n;
		}
...
header('location:../upload.php?s='.$sem.'&dept='.$dept.'&e=1');
?>
</body>
</html>