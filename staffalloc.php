...
font-size: 16px;">&nbsp; <a href="logout.php" class="btn btn-danger square-btn-adjust">Logout</a> </div>
        ...
        <div id="page-wrapper" >
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                     <h2>Staff Allocator</h2>   
                        <h5>Staffs that are available</h5>
                       
                    </div>
                </div>
                <?php
                if($out=$db->query("select distinct dept from teacher where available=1"))
                {
                  if($out->num_rows==0)
                  {
                    echo " <div class=\"form-control\" style=\"color:red\">No department has updated there available staff list</div></br>";
								  die();
                  }
                  while($res=$out->fetch_assoc())
                  {
                    $dept=$res['dept'];
                    $n=$db->query("select branch from dept where abb='$dept'");
                    $name=$n->fetch_assoc();
                    echo "<div class=\"form-control\" style=\"color:blue\">".$name['branch']." dept has submitted there available staff list.</div></br>";
                  }
                }
				?>
                 <!-- /. ROW  -->
                 <hr />
               <a href="teacher.php"><button type="submit" style="margin-left: 42%" class="btn btn-primary">Gen. Staff Allocation</button></a>
    </div>
             <!-- /. PAGE INNER  -->
            </div>
         <!-- /. PAGE WRAPPER  -->
